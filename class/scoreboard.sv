			...
			if(tr_in.enable) begin
				...
				if(!tr_in.rd_wr) begin //Write = 0
					...
					case(tr_in.addr)
						...
						2'b10:  next_rd_val = temp_op;
						2'b11:  next_rd_val = temp_execute;
						default: $display("[Scoreboard] Write to Addr: %0d", tr_in.addr);
					endcase	
	...
	task out();
	    forever begin
			...
			if(flag) begin
				...
				if(tr_out.res_out > 65025) begin
					$display("[Scoreboard] Error OVERFLOW!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! res_out Get: %0d", tr_out.res_out);
					error++;
				end
			end
		
		
			if(tr_out.rd_data > 255) begin
				$display("[Scoreboard] Error OVERFLOW!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! rd_data Get: %0d", tr_out.rd_data);
				error++;
			end
